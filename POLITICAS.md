# Políticas de la tienda — borradores

Estos textos **no viven en el tema**. Van en el admin de Shopify:
**Settings → Policies** (Configuración → Políticas).

El tema solo las lee (`shop.privacy_policy`, `shop.refund_policy`,
`shop.shipping_policy`, `shop.terms_of_service`) y las enlaza desde el footer.

## Cómo usarlos

1. Settings → Policies
2. Pegar cada texto en su campo
3. Reemplazar todo lo que está entre `[CORCHETES]` — son datos que no tengo
4. Guardar

## ⚠️ Términos del Servicio — ver archivo aparte

Lo que está **publicado en vivo** no es este borrador: es la plantilla del
generador de Shopify sin editar, con marcadores `[INSERTAR ...]` visibles para
los clientes. El texto corregido y listo para pegar está en
**`POLITICAS-terminos-actualizado.md`**, junto con los 3 datos registrales que
todavía faltan (razón social, cédula jurídica, IVA).

La Política de Devoluciones publicada también tiene
`[INSERTAR DIRECCIÓN DE DEVOLUCIÓN]` en vivo.

## Antes de publicar

**No soy abogado y esto es un suplemento alimenticio vendido en Costa Rica.**
Los textos son genéricos y sirven de base, pero conviene revisión legal antes de
lanzar — sobre todo devoluciones de producto consumible y cualquier afirmación
de salud.

**Consistencia:** las páginas de producto ya prometen *garantía de satisfacción
de 30 días* y *envío gratis en pedidos superiores a $50*. Los borradores respetan
esas cifras. Si cambiás una, hay que cambiarla en los dos lados.

---

# 1. Política de Privacidad

En Feel Good Pharma respetamos tu privacidad y protegemos los datos personales
que nos confiás. Esta política explica qué información recopilamos, cómo la
usamos y cuáles son tus derechos.

## Qué información recopilamos

Cuando navegás o comprás en nuestro sitio podemos recopilar:

- **Datos de identificación:** nombre, correo electrónico, teléfono y dirección
  de envío y facturación.
- **Datos de la compra:** productos adquiridos, monto, medio de pago y estado
  del pedido. El procesamiento del pago lo realiza [PASARELA DE PAGO]; nosotros
  no almacenamos los datos completos de tu tarjeta.
- **Datos de navegación:** dirección IP, tipo de dispositivo y navegador, páginas
  visitadas y tiempo de permanencia, recopilados mediante cookies.

## Para qué la usamos

- Procesar y entregar tus pedidos.
- Comunicarnos con vos sobre el estado de una compra o una consulta.
- Enviarte información comercial y promociones, únicamente si diste tu
  consentimiento. Podés retirarlo cuando querás.
- Mejorar nuestro sitio, productos y atención.
- Cumplir con obligaciones legales y contables.

## Con quién la compartimos

No vendemos ni alquilamos tus datos personales. Los compartimos solo con:

- Proveedores que hacen posible la operación (plataforma de comercio,
  procesamiento de pagos, mensajería y envíos, herramientas de correo).
- Autoridades competentes, cuando exista una obligación legal.

## Cookies

Usamos cookies propias y de terceros para recordar tu sesión y tu carrito,
entender cómo se usa el sitio y mostrar publicidad relevante. Podés
administrarlas o bloquearlas desde la configuración de tu navegador; algunas
funciones del sitio pueden dejar de operar correctamente si las desactivás.

## Tus derechos

Podés solicitar en cualquier momento acceder a tus datos, corregirlos,
actualizarlos o pedir su eliminación, así como revocar el consentimiento para
recibir comunicaciones comerciales. Escribinos a
[CORREO DE PRIVACIDAD — sugerido: hola@feelgoodpharma.com] y te respondemos
dentro de [PLAZO — sugerido: 10 días hábiles].

El tratamiento de datos personales se rige por la Ley N.º 8968 de Protección de
la Persona frente al Tratamiento de sus Datos Personales de Costa Rica.

## Conservación y seguridad

Conservamos tus datos mientras exista una relación comercial y durante los
plazos que exija la legislación. Aplicamos medidas de seguridad razonables para
protegerlos, aunque ningún sistema es completamente infalible.

## Cambios

Podemos actualizar esta política. La versión vigente siempre estará publicada en
este sitio con su fecha de última modificación.

**Responsable del tratamiento:** [RAZÓN SOCIAL], cédula jurídica
[CÉDULA JURÍDICA], [DIRECCIÓN FISCAL].
**Última actualización:** [FECHA]

---

# 2. Política de Devoluciones y Reembolsos

Queremos que estés conforme con tu compra. Si algo no salió como esperabas,
escribinos y lo resolvemos.

## Garantía de satisfacción de 30 días

Contás con **30 días naturales** desde que recibís tu pedido para solicitar una
devolución. Si no quedás conforme, contactanos y gestionamos el reembolso.

## Condiciones

Para que una devolución proceda, el producto debe estar:

- En su empaque original, con etiquetas y sellos.
- Sin usar o con [CONDICIÓN — ej. menos del 25% de contenido consumido].
- Acompañado del comprobante de compra o número de pedido.

Por tratarse de un suplemento alimenticio, **no podemos aceptar devoluciones de
productos abiertos o parcialmente consumidos**, salvo defecto de fábrica o error
en el envío. Esto protege la inocuidad de lo que vendemos.

## Cómo solicitarla

1. Escribinos a [CORREO] o por WhatsApp al +506 7109 0960.
2. Indicá tu número de pedido y el motivo.
3. Te confirmamos si procede y te damos las instrucciones de envío.

No enviés el producto antes de recibir nuestra confirmación.

## Costos de envío del retorno

- **Defecto de fábrica, producto incorrecto o dañado:** el costo corre por
  nuestra cuenta.
- **Cambio de opinión:** el costo corre por cuenta de la persona compradora.
  [CONFIRMAR]

## Reembolsos

Una vez recibido y revisado el producto, te notificamos si la devolución fue
aprobada. Si procede, el reembolso se realiza al mismo medio de pago original
dentro de [PLAZO — sugerido: 10 días hábiles]. El tiempo en que se refleje
depende de tu banco o emisor.

Los costos de envío originales [SON / NO SON] reembolsables. [CONFIRMAR]

## Producto dañado o incorrecto

Si tu pedido llegó dañado, incompleto o no corresponde a lo que ordenaste,
escribinos dentro de [PLAZO — sugerido: 5 días hábiles] con fotografías del
producto y del empaque. Lo reponemos o reembolsamos sin costo para vos.

## Cambios

Para cambiar un producto por otro, gestionamos la devolución y una nueva compra.
Escribinos y te acompañamos en el proceso.

---

# 3. Política de Envíos

## Cobertura

Realizamos envíos a todo el territorio de Costa Rica.
[CONFIRMAR si hay envíos internacionales — el sitio muestra el mensaje
"Ya disponible con envío a EE. UU."]

## Costos

- **Envío gratis** en pedidos superiores a **$50**.
- Pedidos por debajo de ese monto: [TARIFA].
- [ZONAS CON TARIFA DIFERENCIADA, si aplica]

El costo exacto se calcula al finalizar la compra, antes del pago.

## Tiempos de entrega

- **Entrega estándar:** de **3 a 5 días hábiles** dentro del Gran Área
  Metropolitana.
- **Zonas rurales o alejadas:** [PLAZO].
- Los pedidos se procesan en [PLAZO — sugerido: 1 a 2 días hábiles] antes de
  ser despachados.

Los pedidos realizados después de [HORA] o en fines de semana y feriados se
procesan el siguiente día hábil.

## Seguimiento

Cuando tu pedido sea despachado te enviamos un correo con el número de guía para
que lo sigás. Si no lo recibís, revisá tu carpeta de spam o escribinos.

## Demoras

Los tiempos indicados son estimados y pueden verse afectados por condiciones
climáticas, congestión en temporada alta o factores de la empresa de mensajería.
Te mantenemos al tanto ante cualquier contratiempo.

## Dirección incorrecta

Verificá que tu dirección esté completa y correcta antes de confirmar la compra.
Si un pedido se devuelve por datos incorrectos o por ausencia en la entrega, el
costo de un nuevo envío corre por cuenta de la persona compradora.

**Empresa de mensajería:** [TRANSPORTISTA]

---

# 4. Términos de Servicio

## Aceptación

Al navegar y comprar en este sitio aceptás estos términos. Si no estás de
acuerdo, te pedimos no utilizarlo.

## Quiénes somos

Este sitio es operado por [RAZÓN SOCIAL], cédula jurídica [CÉDULA JURÍDICA], con
domicilio en [DIRECCIÓN FISCAL], Costa Rica.

## Productos

Los productos que vendemos son **suplementos alimenticios**. No son
medicamentos y no están destinados a diagnosticar, tratar, curar ni prevenir
ninguna enfermedad.

La información del sitio tiene fines informativos y no sustituye la consulta con
un profesional de la salud. Si estás embarazada, en período de lactancia, tomás
medicamentos o tenés alguna condición médica, consultá a tu médico antes de
usarlos.

Los resultados varían de una persona a otra. Los testimonios reflejan
experiencias individuales y no garantizan resultados equivalentes.

## Precios y disponibilidad

Los precios están expresados en [MONEDA] e incluyen [IVA INCLUIDO / MÁS IVA].
Podemos modificarlos sin previo aviso. La disponibilidad está sujeta a
existencias; si un producto no está disponible después de tu compra, te
avisamos y reembolsamos el monto correspondiente.

Nos reservamos el derecho de limitar cantidades o rechazar un pedido cuando
detectemos un error evidente de precio o una compra con fines de reventa no
autorizada.

## Cuenta

Si creás una cuenta, sos responsable de mantener la confidencialidad de tus
credenciales y de la actividad que ocurra bajo ella. Avisanos de inmediato ante
cualquier uso no autorizado.

## Propiedad intelectual

Las marcas, logotipos, textos, imágenes y contenidos del sitio son propiedad de
[RAZÓN SOCIAL] o se usan con autorización. No pueden reproducirse ni utilizarse
con fines comerciales sin permiso escrito.

## Limitación de responsabilidad

En la medida permitida por la legislación aplicable, no respondemos por daños
indirectos o incidentales derivados del uso del sitio o de los productos, cuando
se hayan usado de forma distinta a la indicada en su etiqueta.

## Legislación aplicable

Estos términos se rigen por las leyes de la República de Costa Rica. Cualquier
controversia se someterá a los tribunales competentes del país.

## Contacto

[CORREO] · WhatsApp +506 7109 0960 · Lun – Vie 8 a.m. – 8 p.m.,
Sáb y Dom 9 a.m. – 4 p.m.

**Última actualización:** [FECHA]

---

## Datos que faltan

Recopilados de todos los `[CORCHETES]`:

| Dato | Dónde aparece |
|---|---|
| Razón social y cédula jurídica | Privacidad, Términos |
| Dirección fiscal | Privacidad, Términos |
| Correo de contacto legal | las 4 |
| Moneda y tratamiento de IVA | Términos |
| Pasarela de pago | Privacidad |
| Empresa de mensajería | Envíos |
| Tarifa bajo $50 y zonas | Envíos |
| Plazos: procesamiento, zonas rurales, hora de corte | Envíos |
| Quién paga el retorno por cambio de opinión | Devoluciones |
| Si el envío original se reembolsa | Devoluciones |
| Condición aceptable de producto abierto | Devoluciones |
| ¿Hay envíos a EE. UU.? | Envíos |

## Inconsistencia detectada

El home muestra **"Ya disponible con envío a EE. UU. 🇺🇸"**, pero no hay ninguna
política que cubra envíos internacionales: ni tarifas, ni tiempos, ni aduanas,
ni quién asume los impuestos de importación. Conviene resolverlo antes de
lanzar, o quitar ese mensaje.
